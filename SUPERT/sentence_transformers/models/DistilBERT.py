...
class DistilBERT(nn.Module):
    ...
    def __init__(self, model_name_or_path: str, max_seq_length: int = 128, do_lower_case: bool = True):
        ...
        if max_seq_length > 510:
            logging.warning("DistilBERT only allows a max_seq_length of 510 (512 with special tokens). Value will be set to 510")
            max_seq_length = 510
        self.max_seq_length = max_seq_length

        self.distilbert = DistilBertModel.from_pretrained(model_name_or_path)
    ...
    def forward(self, features):
        ...
        output_tokens = self.distilbert(input_ids=features['input_ids'], attention_mask=features['input_mask'])[0]
    ...
    def save(self, output_path: str):
        self.distilbert.save_pretrained(output_path)
